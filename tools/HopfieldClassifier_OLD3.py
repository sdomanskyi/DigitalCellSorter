...
class HopfieldClassifier:
    ...
    def CrossValidate(self,
        ...
        res = minimizeCompass(self.CVObjFn, 
                              [self.T], 
                              bounds = ((0.0,0.001),), #bounds = ((0.0,100.0),), 
                              options = options, 
                              tol = 1e-5,
                              xtol = 1e-5,
                              paired = False,
...
#        return T**2.0
        ...
        for testIndices in np.array_split(allIndices,k):
            ...
            predictions = self.predict_proba(self.attrs[:,testIndices].T,
                                             T = T[0],
                                             numRuns=numRuns,
                                             attrs = self.attrs[:,trainIndices],
                                             )[:,1]
            result = (predictions*2.0-1.0) * (self.y_train_bool[testIndices]*2.0-1.0)
            runList.append( np.mean(result) )
        
        output = -(np.mean(runList)+1.0)/2.0
        print 'T=%s // score=%s'%(T[0],output)
